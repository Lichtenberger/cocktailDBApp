...
# cocktail = pandas.read_csv(COCKTAIL_CSV)
# cocktail.columns = [c.replace(' ', '_') for c in cocktail.columns]
# cocktail_attributes = cocktail[['Cocktail_Name', 'Ingredients', 'Preperation']]
...
#     con = psycopg2.connect(database='cocktailsdb', user='postgres', password={password}, host='127.0.0.1')
# else:
...
#     DROP TABLE IF EXISTS cocktailsdb CASCADE;
#     CREATE TABLE cocktailsdb
#     (
#         id SERIAL,
#         name TEXT NOT NULL,
#         prep TEXT,
...
#     cocktail_ingredients = cocktail[['Cocktail_Name', 'Ingredients']]
...
# def db_init():
#     create_db()
#     initial_table_insert(dataset=normalize_rows(column=cocktail.Ingredients), table_name='ingredients')
